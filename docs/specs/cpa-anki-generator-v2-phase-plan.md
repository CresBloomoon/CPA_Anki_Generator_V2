...
### Phase6-2: サンプル実PDFでのE2E手動検証 — 完了（実運用での確認）
...
任意Phaseに着手してよい）
- **完了記録（本Phaseとして個別に実施したのではなく、実運用での確認により完了と確定）**：
  専用の検証セッションは設けていないが、Step7時点までに、まーくん自身が実際のCPA教材PDFで
  何度もカード生成→Anki Desktopへのインポート→日常的な使用を繰り返しており、回答カードの
  クリップボード画像コピー機能（Phase2-6でhtml2canvas／_success_icon.pngを同梱して修正した
  部分）を含め、上記チェックリストの内容はいずれも問題なく動作することを確認済み。よって
  Phase6-2は実運用での確認をもって完了として扱う。
...
---
（全36Phaseがタスクとして登録済み。任意Phaseのうち2-8/2-9/5-12は後日実施し完了済み。
0-4（Lint/Format整備）／2-7（科目自動推定リポジトリ）／5-13（ルートパス欄への科目自動
サジェスト、2-7に連動）の3件は、今後も対応不要として見送りを確定した——まーくんとの合意事項）
